<? 
    ...
    $row = $my->gets("SELECT * FROM gallery ORDER BY uid DESC LIMIT {$start},{$limit}");
    $tt = $my->get("SELECT count(uid) AS c FROM gallery");
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header text-secondary pb-0">
                <div class="float-start">Gallery</div>
            ...
            <div class="card-body">
                <table class="w-100 text-center text-sm">
                    ...
                    <?
                        if(!$row){
                            ?><tr class='border-bottom'><td class='py-3' colspan='6'>No Gallery</td></tr><?
                        }
                        $n=$tt['c'];
                        foreach($row AS $v){
                            if($v['thumb_s']){
                                $dir_img    =   "/asset/upload/gallery/".$v['thumb_s'];
                                $style      =   'width:100%; opacity:1;';
                            }else{
                                $dir_img    =   "/asset/img/logo_bk.png";
                            ...
                            <tr class="border-bottom">
                                ...
                                <td class="p-2"><?=date('Y-m-d',$v['i_stamp'])?></td>
                                <td class="p-2">
                                    <a href="edit.php?id=<?=$v['uid']?>" class="btn btn-sm bg-white border mb-0 px-3">Edit</a>
                                </td>
                                <td class="p-2">
                                <a href="javascript:void(0);" onclick="gallery_del_proc('<?=$v['uid']?>')" class="btn btn-sm btn-danger border mb-0 px-3">Del</a>
                                </td>
                            </tr>
                            <?
                            $n--;
...
<script type='text/javascript'>
    //
    gallery_del_proc = function(id){
        if(confirm('해당 gallery를 삭제하시겠습니가?')){
